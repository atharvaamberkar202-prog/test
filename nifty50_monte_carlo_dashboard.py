...
LOOKBACK = st.sidebar.slider("Lookback Years",3,10,5)
...
def load_data():
    ...
    start = end - datetime.timedelta(days=365*LOOKBACK)
...
# -----------------------------
# GLOBAL RETURNS DATAFRAME (FIXED)
# -----------------------------
...
for k,v in returns.items():
    ...
    if k != "NIFTY" and isinstance(v,pd.Series) and len(v)>0:
...
if len(global_series)==0:
    ...
    st.error("No global market data available")
    st.stop()


global_df = pd.concat(global_series,axis=1).dropna()


# -----------------------------
# PCA GLOBAL FACTOR MODEL
# -----------------------------
...
def get_sentiment():
    ...
    if NEWS_API:
        ...
        try:
            ...
            url=f"https://newsapi.org/v2/everything?q=nifty india stock market&language=en&pageSize=20&apiKey={NEWS_API}"
...
st.metric("Probability NIFTY Up Tomorrow",f"{prob_up:.2f}%")
